...
//First argument is the origin folder
//Second argument is the target folder
if(process.argv.length != 4)
{
	console.log("missing parameters")
	return;
}
const originFolder = process.argv[2];
		...
		fs.readdir(originFolder, function (err, files) {
			...
			files.forEach(function (file) {
				console.log(file);
					...
					fs.readFile(`${originFolder}/${file}`, function(err, data) {
						if (err) {
						...
						// Encode mesh
						encodeMeshToFile(decodedGeometry, decoder,`${targetFolder}/${file}`);
...
function encodeMeshToFile(mesh, decoder,name) {
	...
	// example.
	fs.writeFile(name, Buffer.from(outputBuffer), "binary",
		function(err) {
			if (err) {
				console.log(err);
			} else {
